...
needs and, with no password set, the tail of the last install log. While a
failed restore holds the boot, `/api/` answers `503` with `installing: false`
and `restore_failed: true`.
