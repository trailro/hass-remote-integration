...
for f in tests jsonio.py backupkit.py logbuffer.py entrypoint.py run.py verify.sh docker-compose.yml Dockerfile README.md SECURITY.md .github tools test_components.py; do docker cp -q "$f" "$TEST_CONTAINER:$D/"; done && docker cp -q custom_components/integration_manager "$TEST_CONTAINER:$D/custom_components/"
