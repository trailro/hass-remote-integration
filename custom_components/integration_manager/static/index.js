...
function renderIntegrations(s){
  ...
  t.querySelectorAll('button[data-a]').forEach(b=>b.onclick=async()=>{const d=b.dataset.d, a=b.dataset.a;
    if(a==='start'){ const tag=t.querySelector(`select[data-sel="${d}"]`).value; const other=RUN&&RUN.domain&&RUN.domain!==d?` ${RUN.domain} is stopped first (its entries disabled).`:'';
      if(!confirm(`Start ${d} ${tag}?${other} A preflight runs first unless it is the version already deployed or a dev build. Its files are deployed, requirements installed, patches applied, config entries the manager disabled enabled again (entries you disabled stay disabled); MQTT identity becomes hass_${d}.`)) return;
    ...
    if(a==='uninstall'){ if(!confirm(`Uninstall ${d}? Every version, the deployed files and its config entries are removed.`)) return; const r=await post(`api/installed/${d}/uninstall`); const on=r.retained_cleanup_other_broker?`once MQTT is set to ${r.retained_cleanup_broker} again`:''; log(r.ok?`uninstalled ${d}`+(r.retained_cleanup_deferred?`; MQTT is disabled: its retained data on ${r.retained_cleanup_broker} is removed once MQTT is enabled${on?' and set to that broker':''}`:r.retained_cleanup_failed?`; MQTT cleanup on ${r.retained_cleanup_broker} failed (${r.retained_cleanup_error||'broker unreachable'}): retried ${on||'every minute'}`:''):'ERROR: '+r.error); await status(); await mqttSummary(); }
  });
}
