...
function renderIntegrations(s){
  ...
  t.querySelectorAll('button[data-a]').forEach(b=>b.onclick=async()=>{const d=b.dataset.d, a=b.dataset.a;
    ...
    if(a==='uninstall'){ if(!confirm(`Uninstall ${d}? Every version, the deployed files and its config entries are removed.`)) return; const r=await post(`api/installed/${d}/uninstall`); log(r.ok?`uninstalled ${d}`+(r.retained_cleanup_failed?`; MQTT cleanup failed (${r.retained_cleanup_error||'broker unreachable'}): retried every minute`:''):'ERROR: '+r.error); await status(); await mqttSummary(); }
  });
}
