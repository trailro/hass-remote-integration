...
function renderIntegrations(s){
  ...
  t.querySelectorAll('button[data-a]').forEach(b=>b.onclick=async()=>{const d=b.dataset.d, a=b.dataset.a;
    if(a==='start'){ const tag=t.querySelector(`select[data-sel="${d}"]`).value; const other=RUN&&RUN.domain&&RUN.domain!==d?` ${RUN.domain} is stopped first (its entries disabled).`:'';
      ...
      log(`starting ${d} ${tag}…`); const r=await post('api/run/start',{domain:d,tag}); log(r.ok?`started ${r.domain} ${r.tag}${r.restart_required?' — RESTART REQUIRED to load the new code':''}${r.pip_failed&&r.pip_failed.length?' · pip failed: '+r.pip_failed.join(', '):''}${r.mqtt&&r.mqtt.connect_error?' · MQTT: '+r.mqtt.connect_error:''}`:'ERROR: '+r.error); await status(); await mqttSummary(); }
...
status().catch(e=>log('error: '+e)); mqttSummary().catch(()=>{});
setInterval(status,15000); setInterval(mqttSummary,15000);
