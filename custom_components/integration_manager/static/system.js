...
$('#imapplyall').onclick=async()=>{ const doms=Object.keys(IMS.domains).filter(d=>INSTALLED[d]&&IMS.domains[d].importable); if(!doms.length) return;
  ...
  $('#imallresult').textContent=r.ok?`imported: ${r.imported.map(x=>x.domain+' ('+x.state+')').join(', ')||'none'}; skipped: ${r.skipped.map(x=>x.domain).join(', ')||'none'}; failed: ${r.failed.map(x=>x.domain+': '+x.error).join('; ')||'none'}; extracted backup removed`:'ERROR: '+r.error; if(r.ok){IMS=null;IMSEL=null;imRender();} sysStatus(); };
