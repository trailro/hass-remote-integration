async function mqtt(){
  ...
  $('#mqconn').innerHTML=(s.enabled?(s.connected?`<span class="ok">connected</span>${s.protocol?' <span class="mut">'+esc(s.protocol)+'</span>':''}`:`<span class="bad">disconnected</span> ${s.connect_error?'<span class="err">'+esc(s.connect_error)+'</span>':''}`):'<span class="warn">disabled</span>')+skipped;
