async function mqtt(){
  ...
  $('#mqrules').innerHTML=`${s.rules||0} per-entity rule(s): exclude, name, icon, category, device class and disabled-by-default, on the MQTT side only · edit per entity on the <a href="/entities" style="color:#58a6ff">entities page</a> or <a href="#" id="mqrulesedit" style="color:#58a6ff">as JSON</a> (globs like <code>sensor.*_rssi</code> allowed)`;
  ...
  $('#mqcmd').textContent=`${s.commands} · last: ${esc(s.last_command||'—')}${s.cmd_base?` · topic: ${esc(s.cmd_base)}/<domain>/<object_id>/<field>`:''}`;
