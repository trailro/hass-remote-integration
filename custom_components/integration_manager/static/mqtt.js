async function mqtt(){
  ...
  const h=s.health||{}; $('#mqhealth').innerHTML=`<span class="${h.state==='ok'?'ok':h.state==='stopped'?'mut':h.state==='degraded'?'warn':'bad'}">${esc(h.state||'—')}</span>${h.reason?' · '+esc(h.reason):''} · ${s.health_topic?`<code>${esc(s.health_topic)}</code> retained, refreshed every 60 s`:'no health topic until an integration runs'}${h.integration?` · ${esc(h.integration)} ${esc(h.version||'')}: ${h.entities_with_state??'?'} entities with a state, ${h.entities_unavailable??0} unavailable, last update ${h.last_state_update_age_s!=null?h.last_state_update_age_s+' s ago':'never'}`:''} · manager device on the consuming HA (${s.has_identity?`<code>binary_sensor.${esc(s.base_topic)}_integration</code>, <code>sensor.${esc(s.base_topic)}_health</code>, `:''}update entities, resource sensors): ${s.discovery_enabled?'<span class="ok">on</span> (with discovery)':s.manager_discovery?'<span class="ok">on</span> (manager_discovery)':'<span class="mut">off</span>'}${s.manager_commands?' · actions <span class="warn">on</span>':''}${s.manager_topic?` · <code>${esc(s.manager_topic)}</code>`:''}`;
  $('#mqdisc').innerHTML=(s.discovery_enabled?`<span class="ok">on</span>`:'<span class="warn">off</span> (keep it off while running in shadow mode)')+` · prefix <code>${esc(s.discovery_prefix)}</code> · ${s.discovery_devices} devices, ${s.discovery_components} components (${s.discovery_mirrored} mirrored as sensor, ${s.discovery_disabled} disabled) · ${s.main_ha_version?`main HA <code>${esc(s.main_ha_version)}</code>: ${s.discovery_compat_device_classes_dropped||0} device class(es) left out, ${s.discovery_compat_platforms_mirrored||0} entity/entities mirrored as sensor for it`:'main HA version not declared: nothing left out'} · <a href="/api/mqtt/discovery" style="color:#58a6ff">preview</a>`;
  // .textContent parses no HTML: esc() here only put &quot; and &amp; on screen
  $('#mqcmd').textContent=`${s.commands} · last: ${s.last_command||'—'}${s.cmd_base?` · topic: ${s.cmd_base}/<domain>/<object_id>/<field>`:''}`;
...
async function healthRules(){
  ...
  for(const d of Object.keys(INSTALLED)){ const own=(r.health||{})[d]||{}; const tr=document.createElement('tr');
    tr.innerHTML=`<td><b>${esc(d)}</b>${RUN&&RUN.domain===d?' <span class="tag ok">running</span>':''}</td><td><select data-h="mode" data-d="${esc(d)}"><option value="">periodic (default)</option><option value="periodic" ${own.mode==='periodic'?'selected':''}>periodic</option><option value="event" ${own.mode==='event'?'selected':''}>event</option></select></td>
     <td><select data-h="stale_basis" data-d="${esc(d)}"><option value="">reported (default)</option><option value="reported" ${own.stale_basis==='reported'?'selected':''}>reported</option><option value="updated" ${own.stale_basis==='updated'?'selected':''}>updated</option></select></td>
    ...
    t.appendChild(tr); }
  t.querySelectorAll('button[data-hs]').forEach(b=>b.onclick=async()=>{const d=b.dataset.hs; const rules={}; t.querySelectorAll(`[data-d="${d}"]`).forEach(el=>{ if(el.value!=='') rules[el.dataset.h]=el.tagName==='SELECT'?el.value:parseInt(el.value,10); });
