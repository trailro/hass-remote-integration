async function mqtt(){
  ...
  const h=s.health||{}; $('#mqhealth').innerHTML=`<span class="${h.state==='ok'?'ok':h.state==='stopped'?'mut':h.state==='degraded'?'warn':'bad'}">${esc(h.state||'—')}</span>${h.reason?' · '+esc(h.reason):''} · <code>${esc(s.health_topic||'')}</code> retained, refreshed every 60 s${h.integration?` · ${esc(h.integration)} ${esc(h.version||'')}: ${h.entities_with_state??'?'} entities with a state, ${h.entities_unavailable??0} unavailable, last update ${h.last_state_update_age_s!=null?h.last_state_update_age_s+' s ago':'never'}`:''} · discovery adds <code>binary_sensor.${esc(s.base_topic)}_integration</code> (connectivity) + <code>sensor.${esc(s.base_topic)}_health</code> on the consuming HA`;
...
const MQ_TEXT=['host','username','discovery_prefix'], MQ_INT={port:1883,republish_interval_s:300,full_republish_interval_min:60}, MQ_BOOL=['enabled','discovery_enabled','force_base_topic'];
