...
function fieldInput(name,f){const sel=f.selector||{}, k=Object.keys(sel)[0], v=sel[k]||{}, ex=f.example!==undefined?f.example:(f.default!==undefined?f.default:'');
 ...
 // the values the example (or the default) stands for: valueList (hri.js) drops the ones that are not there
 // before stringifying them, so a field whose example is null is not drawn -- and sent -- as the word "null"
 const pre=new Set(valueList(ex===''?[]:ex));
 if(opts&&v.multiple){  // a list, as the selector declares: checkboxes, and custom items when allowed
